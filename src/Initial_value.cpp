...
void Initial_value()
{
   ...
   TFile *outputfile = new TFile("/Users/taku/Downloads/survey_data/Initial_value.root","recreate");
   pm3->Write();
   pm3h->Write();
   ...
   ex_flag = 0;
   std::ofstream PixelPosition("outputfiles/PixelPosition_2019USsurvey.dat");
   for(Int_t i=0; i<512; i++){
      for(Int_t j=0; j < ex_size; j++){
         if(511 - i == ex_id[j]) ex_flag = 1;
      }
