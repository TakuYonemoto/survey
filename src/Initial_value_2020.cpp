...
void Initial_value_2020()
{
   ...
   ifs.open("outputfiles/FARO_COBRA_calib2020.csv");
   if(!ifs){
      cout << "Error: File cannot open!" << endl;
   } 
   while(getline(ifs,str)){
   ...
   TFile *boxmake1 = new TFile("/Users/taku/Downloads/survey_data/boxmake1.root","read");
   ...
   TFile *outputfile = new TFile("/Users/taku/Downloads/survey_data/Initial_value_2020.root","recreate");
   pm3->Write();
   pm3h->Write();
   ...
   std::ofstream geometry2020("outputfiles/Geometry2020.csv");
   geometry2020 << "id \t idx \t XYZPosition_0 \t XYZPosition_1 \t XYZPosition_2 \t XPhi \t XTheta \t XPsi \t Order \t GeometricalHitOrder" << std::endl;
   for(Int_t i=0; i<512; i++){
      if(i < 256){
         geometry2020 << id_geo[i]+1 << "\t" << idx_geo[i] << "\t" << v_geo[i].X()/10. << "\t" << v_geo[i].Y()/10. <<
      ...
      if(i >= 256){
         ...
         if(ex_flag){
            ...
            geometry2020 << id_geo[i]+1 << "\t" << idx_geo[i] << "\t" << (v_geo[i].X() - x_mean_trans[i-256])/10. <<
   ...
   ex_flag = 0;
   std::ofstream PixelPosition("outputfiles/PixelPosition_2020USsurvey.dat");
   for(Int_t i=0; i<512; i++){
      for(Int_t j=0; j < ex_size; j++){
         if(511 - i == ex_id[j]) ex_flag = 1;
      }
      if(i >= 256 && ex_flag != 1){
         x = v_geo[i].X() - x_mean_trans[i-256];
         y = v_geo[i].Y() - y_mean_trans[i-256];
         z = v_geo[i].Z() - z_mean_trans[i-256];
   ...
   std::ofstream Rndm_dat("outputfiles/Rndm5.dat");
   for(Int_t i=0; i<512; i++){
      Rndm_dat << Rndm[3*i] << "\t" << Rndm[3*i+1] << "\t" << Rndm[3*i+2] << std::endl;
   }
   Rndm_dat.close();

   std::ofstream Rndms("outputfiles/Rndm5.csv");
   ...
   for(Int_t i=0; i<512; i++){
      ...
      Rndms << id_geo[i]+1 << "\t" << idx_geo[i] << "\t" << x << "\t" << y <<
         "\t" << z << "\t" << phi << "\t" << theta_geo[i] << "\t" << psi_geo[i] <<
         "\t" << order_geo[i] << "\t" << hitorder_geo[i] << std::endl;
   }
   Rndms.close();
   */
