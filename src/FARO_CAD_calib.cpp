...
{
   //Geometrical Method    
   TVector3 v1_F, v2_F, v3_F;
   TVector3 v1_C, v2_C, v3_C;

   v1_F = old1_FARO - old4_FARO;
   v1_C = old1_CAD  - old4_CAD;
   v2_F = old2_FARO - old4_FARO;
   v2_C = old2_CAD  - old4_CAD;
   v3_F = old3_FARO - old4_FARO;
   v3_C = old3_CAD  - old4_CAD;

   //-----rotate-----
   TVector3 v_Rot, v_Tar, v_Ax, v_Ax2;
   
   v_Rot = v1_F * (1./v1_F.Mag());
   v_Tar = v1_C * (1./v1_C.Mag());
   v_Ax2 = v_Tar;
   Double_t theta1 = ACos(v_Rot * v_Tar);
   v_Ax = v_Rot.Cross(v_Tar);
   v_Rot = v2_F * (1./v2_F.Mag());
   v_Tar = v2_C * (1./v2_C.Mag());
   v_Rot.Rotate(theta1, v_Ax);
   Double_t theta2_about_v1 = 2*ATan(((v_Rot - v_Tar).Mag())/((v_Ax2.Cross(v_Rot + v_Tar)).Mag()));
   ...
   v_Rot = v1_F;
   v_Rot.Rotate(theta1, v_Ax);
   v_Rot.Rotate(theta2_about_v1, v_Ax2);
   v_Geo = v_Rot - v1_C;
   std::cout << " 1 : " << v_Geo.X() << " , " << v_Geo.Y() << " , " << v_Geo.Z() << std::endl;
   Loss += v_Geo.Mag2();
   v_Rot = v2_F;
   v_Rot.Rotate(theta1, v_Ax);
   v_Rot.Rotate(theta2_about_v1, v_Ax2);
   v_Geo = v_Rot - v2_C;
   std::cout << " 2 : " << v_Geo.X() << " , " << v_Geo.Y() << " , " << v_Geo.Z() << std::endl;
   Loss += v_Geo.Mag2();
   v_Rot = v3_F;
   v_Rot.Rotate(theta1, v_Ax);
   v_Rot.Rotate(theta2_about_v1, v_Ax2);
...
// Fitting Method
   ...
   Double_t vstart[] = {376.,-2.9,-1142.,359,180,190,0.1};
   ...
   std::cout << "=======================================" << std::endl;
   check(par_result);
   std::ofstream output_file("outputfiles/FARO_CAD_calib.csv");
   for(Int_t i=0; i < npar; i++){
...
void check(Double_t *par)
{
   ...
   v_Rot.RotateZ(par[5]*TMath::DegToRad());
   v_Res = -old3_CAD + v_Rot;
   std::cout<<" old3_1.5 : "<< v_Res.X()<<", "<<v_Res.Y()<<", "<<v_Res.Z()<<std::endl;
   ...
   v_Rot.RotateZ(par[5]*TMath::DegToRad());
   v_Res = -old4_CAD + v_Rot;
   std::cout<<" old4_1.5 : "<< v_Res.X()<<", "<<v_Res.Y()<<", "<<v_Res.Z()<<std::endl;
   ...
   v_Rot.RotateZ(par[5]*TMath::DegToRad());
   v_Res = -old1_CAD + v_Rot;
   std::cout<<" old1_0.5 : "<< v_Res.X()<<", "<<v_Res.Y()<<", "<<v_Res.Z()<<std::endl;
   v_Rot = old2_FARO2 - v_Para;
   v_Rot.RotateX(par[3]*TMath::DegToRad());
   v_Rot.RotateY(par[4]*TMath::DegToRad());
   v_Rot.RotateZ(par[5]*TMath::DegToRad());
   v_Res = -old2_CAD + v_Rot;
   std::cout<<" old2_0.5 : "<< v_Res.X()<<", "<<v_Res.Y()<<", "<<v_Res.Z()<<std::endl;
   v_Rot = old3_FARO2 - v_Para;
   v_Rot.RotateX(par[3]*TMath::DegToRad());
   v_Rot.RotateY(par[4]*TMath::DegToRad());
   v_Rot.RotateZ(par[5]*TMath::DegToRad());
   v_Res = -old3_CAD + v_Rot;
   std::cout<<" old3_0.5 : "<< v_Res.X()<<", "<<v_Res.Y()<<", "<<v_Res.Z()<<std::endl;
   v_Rot = old4_FARO2 - v_Para;
   v_Rot.RotateX(par[3]*TMath::DegToRad());
   v_Rot.RotateY(par[4]*TMath::DegToRad());
   v_Rot.RotateZ(par[5]*TMath::DegToRad());
   v_Res = -old4_CAD + v_Rot;
   std::cout<<" old4_0.5 : "<< v_Res.X()<<", "<<v_Res.Y()<<", "<<v_Res.Z()<<std::endl;
