...
TFile *fo = new TFile("migrad_401.root","recreate");
...
void init2() {
tree2->SetBranchAddress("cx1",cx);
tree2->SetBranchAddress("cy1",cy);
tree2->SetBranchAddress("cz1",cz);
tree2->SetBranchAddress("tx1",tx);
tree2->SetBranchAddress("ty1",ty);
tree2->SetBranchAddress("tz1",tz);
tree2->GetEntry(0);
}
...
void migrad_401()
{
   ...
   for(Int_t i=0; i<N; i++){
      ...
      y = v.Y();
      z = v.Z();
      if(y <= -20. && fabs(ATan(z/y)) < ATan(2.1/20.)
           && fabs(ATan(x/y)) < ATan(60./20.)){
         tree1->Fill();
      }
      if(x <= -60. && fabs(ATan(z/x)) < ATan(2.1/60.)
            && fabs(ATan(y/x)) < ATan(20./60.)){
         tree1->Fill();
      }
      if(y <= 0. && fabs(ATan(x/z)) < ATan(60/2.5)
            && fabs(ATan(y/z)) < ATan(20./2.5)
            && z >= 3.1 && z <= 3.2){
         tree1->Fill();
      }
   }
   ...
   cout<<"***********************************"<<endl;
   tree->Branch("cx",&cx[9],"cx/D");
   tree->Branch("cy",&cy[9],"cy/D");
   tree->Branch("cz",&cz[9],"cz/D");
   tree->Branch("tx",&tx[9],"tx/D");
   tree->Branch("ty",&ty[9],"ty/D");
   tree->Branch("tz",&tz[9],"tz/D");
   ...
   ggg->SetNameTitle("ggg","counter_401_after_PreTrans; w[mm]; v[mm]; u[mm]");
   TGraph2D *gg = new TGraph2D();
   gg->SetNameTitle("gg","counter_401_after_Fit; w[mm]; v[mm]; u[mm]");
   Long64_t Ngg = 0;
   ...
   legend->AddEntry("ggg","Scanned data of counter 401","P");
   ...
   legend2->AddEntry("gg","Scanned data of counter 401","P");
