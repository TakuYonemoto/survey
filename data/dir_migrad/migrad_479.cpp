...
void migrad_479()
{
   ...
   for(Int_t i=0; i<N; i++){
      ...
      y = v.Y();
      z = v.Z();
      if(i % 3 == 0){ggg->SetPoint(Ngg++,x,y,z);}
      if(y <= -20. && fabs(ATan(z/y)) < ATan(1.5/20.)
           && fabs(ATan(x/y)) < ATan(60./20.)){
         tree1->Fill();
      }
      if(x <= -60. && fabs(ATan(z/x)) < ATan(1.5/60.)
            && fabs(ATan(y/x)) < ATan(18./60.)){
         tree1->Fill();
      }
      if(y <= 5. && fabs(ATan(x/z)) < ATan(40/2.5)
            && fabs(ATan(y/z)) < ATan(13./2.5)
            && z >= 4.1 && z <= 4.2){
         tree1->Fill();
      }
   }
