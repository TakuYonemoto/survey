...
TFile *fo = new TFile("migrad_400.root","recreate");
...
void init2() {
tree2->SetBranchAddress("cx0",cx);
tree2->SetBranchAddress("cy0",cy);
tree2->SetBranchAddress("cz0",cz);
tree2->SetBranchAddress("tx0",tx);
tree2->SetBranchAddress("ty0",ty);
tree2->SetBranchAddress("tz0",tz);
tree2->GetEntry(0);
}
...
void migrad_400()
{
   ...
   ggg->SetNameTitle("ggg","counter_400_after_PreTrans; w[mm]; v[mm]; u[mm]");
   Int_t Nggg = 0;
   for(Int_t i=0; i<N; i++){
      t1->GetEntry(i);
      x = x - cx[9];
      ...
      y = v.Y();
      z = v.Z();
      if(i%3 == 0){ggg->SetPoint(Nggg++,x,y,z);}
      if(y <= -20. && fabs(ATan(z/y)) < ATan(2./20.)
      ...
      if(y <= 0. && fabs(ATan(x/z)) < ATan(55/2.5)
   ...
   gg->SetNameTitle("gg","counter_400_after_Fit; w[mm]; v[mm]; u[mm]");
   Long64_t Ngg = 0;
   ...
   legend->AddEntry("ggg","Scanned data of counter 400","P");
   ...
   legend2->AddEntry("gg","Scanned data of counter 400","P");
